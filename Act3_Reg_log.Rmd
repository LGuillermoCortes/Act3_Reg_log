...
```
```{r, echo = TRUE, warning=FALSE}
sum(is.na(rotacion$Satisfacción_Laboral))
dataset <- dataset[!is.na(dataset$Satisfacción_Laboral), ]
```
...
train_rota <- rotacion[trainIndex, ]
test_rota <- rotacion[-trainIndex, ]
```

```{r, echo = TRUE, warning=FALSE}
model <- train(rotacion ~ ., data = train_rota, method = "glm", family = binomial())
```
